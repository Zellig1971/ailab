...
const CardFooter = (props: CardFooterProps) => {
    ...
    const fetchMd = async () => {
        setLoadingMd(true)
        const file = await mdDynamicImporter({mdPath, mdName, mdPrefixPath, mdType})
    ...
    return (
        <div>
                ...
                <div className="flex items-center gap-2">
                    {
                        expand && (
                            ...
                            >
                                <Segment.Item className='rounded-lg' value="ts">TS</Segment.Item>
                                <Segment.Item  className='rounded-lg' value="js">JS</Segment.Item>
                            </Segment>
                        )
                    }
                    {loadingMd && <Spinner />}
            ...
            <div className={expand ? 'block' : 'hidden'}>
                {mdType === 'ts' && (
                    <CodeBox markdown={tsMarkdown as string} className="text-base" />
                )}
                {mdType === 'js' && (
                    <CodeBox markdown={jsMarkdown as string || tsMarkdown as string} className="text-base" />
                )}
            </div>
        </div>
    )
